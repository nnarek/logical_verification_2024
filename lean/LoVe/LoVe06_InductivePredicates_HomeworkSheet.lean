...
theorem IsLam_lam (s : String) (t : Term) :
  IsLam (Term.lam s t) := IsLam.intro s t
...
theorem TCV3_step {α : Type} (R : α → α → Prop) (a b c : α) (rab : R a b)
  ...
  by
    induction tbc with
    | base a' b' rab' => apply TCV3.trans a a' b' (TCV3.base _ _ rab) (TCV3.base _ _ rab')
    | trans a' b' c' tab' tbc' => apply TCV3.trans a b' c' (a_ih rab) tbc'
...
theorem TCV1_pets {α : Type} (R : α → α → Prop) (c : α) :
  ...
  by
    intro a b tab rbc
    induction tab with
    | base a' b' => exact TCV1.step a' b' c a_1 (TCV1.base b' c rbc)
    | step a' b' c' rab' tbc' => exact TCV1.step a' b' c rab' (a_ih rbc)
